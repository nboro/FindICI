...
from keras.layers import Input, Dense, Embedding, Conv2D, MaxPool2D, Conv1D, MaxPooling1D, GlobalMaxPooling1D,MaxPooling2D, GlobalMaxPooling2D
...
class cnn:
    ...
    def __init__(self, mutated_dst):
        ...
        self.train_set, self.val_set, self.test_set = np.split(self.mutated.sample(frac=1), 
                                                [int(.6*len(self.mutated)), int(.8*len(self.mutated))])
    ...
    def doc2vecskip(self):
        
        vector_size = 100
        tagged_data_tasks_train = [TaggedDocument(d, [i]) for i, d in enumerate(self.train_set['task_complete'])]
        d2v_model_tasks = Doc2Vec(tagged_data_tasks_train, dm = 0, vector_size=vector_size, window=5, min_count=1, alpha = 0.025,
    ...
    def prepare(self):
        ...
        self.train_set['task_complete_one_string'] = self.train_set['task_complete'].apply(lambda x: self.list_to_string(x))
        self.test_set['task_complete_one_string'] = self.test_set['task_complete'].apply(lambda x: self.list_to_string(x))
    ...
    def create_emb_matrix(self,max_tokens,vector_size,d2v_model,tokenizertrain):
        ...
        for word, i in tokenizertrain.word_index.items():
            if i < num_words:
                embedding_vector = d2v_model[word]
    ...
    def model_train(self,num_words,vector_size, embedding_matrix, max_tokens):
        ...
        cnn_model.add(layers.Conv1D(filters = num_filters, kernel_size = max_tokens, activation='relu', padding='same',kernel_regularizer=regularizers.l2(weight_decay)))
        cnn_model.add(layers.MaxPooling1D(2))
        cnn_model.add(Dropout(0.25))

        cnn_model.add(layers.Conv1D(filters = num_filters+num_filters, kernel_size = max_tokens, activation='relu', padding='same',kernel_regularizer=regularizers.l2(weight_decay)))
...
#         adam = Adam(lr=0.001, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=0.0)
    ...
    def run_cnn(self):
        ...
        doc2vec, emb_size = self.doc2vecskip()
        print('Doc2Vec done')
        print('==============')
        ...
        embed_matrix, num_of_words = self.create_emb_matrix(tokens_max, emb_size, doc2vec, train_tokenizer)
...
#         test_tasks_pad=test_tasks_pad.reshape(test_tasks_pad.shape[0],test_tasks_pad.shape[1],1)
        ...
        histry = cnn_model.fit(train_tasks_pad, to_categorical(self.y_train), validation_data=(test_tasks_pad, to_categorical(self.y_test)), epochs=20, 
    ...
    def viz_loss_acc(self,history,module='all'):
        ...
        path_acc = 'skip/cnn/cnn_doc2vec_acc_' + module + '.jpg'
        ...
        path_loss = 'skip/cnn/cnn_doc2vec_loss_' + module + '.jpg'
...
#         val_tasks_padded=val_tasks_padded.reshape(val_tasks_padded.shape[0],val_tasks_padded.shape[1],1)
        ...
        accuracy = round((tp+tn)/all_val,3)
        print('The manual accuracy is ', accuracy)
        ...
        # show the legend
        pyplot.legend()
        path = 'skip/cnn/cnn_doc2vec_roc_'+ module + '.jpg'
        pyplot.savefig(path)
        pyplot.clf()
...
if __name__ == "__main__":
    ...
    for module in top10_list:
        ...
        matthews, accuracy, precision, recall, f1_score, precision_neg, recall_neg, f1_score_neg, roc_auc = obj.get_metrics(cnn_model,
        ...
        mod_results = [accuracy, precision, recall, f1_score, precision_neg, recall_neg, f1_score_neg, matthews,roc_auc]
    ...
    print(results_df)
    results_df.to_excel('skip/cnn_doc2vec.xlsx')