...
#             'learning_rate': ['constant','adaptive'],
        ...
        mlp = MLPClassifier(max_iter = 50, activation = 'relu', alpha = 0.001, learning_rate = 'adaptive')
        
        cv = StratifiedKFold(n_splits=10,
                     shuffle=True,
                     random_state=42)
        
        grid_search = GridSearchCV(estimator = mlp, param_grid = parameter_space, 
                          cv = cv, n_jobs = -1, verbose = 2)
        
        mlp_hyper = grid_search.fit(train_data_vecs, self.y_train)
...
if __name__ == "__main__":
    ...
    for module in top10_list:
        ...
        print('Running MLP for the module ', module)
        
        filtered_mutated = get_filtered_mutated(mutated, module)
    
        rand_obj = mlp(filtered_mutated)

        report_dict, best_params, fpr, tpr, auc, mat = rand_obj.run_mlp()
        
        print('The best estimators for the module ', module, ' are: ', best_params)
        
        file_title = 'skip/mlp_best_params/mlp_doc2vec_best_params_' + module + '.pkl'
        ...
        plt.title('ROC curve')
        plt.legend(loc='best')
        path = 'skip/mlp/mlp_doc2vec_'+ module + '.jpg'
        plt.savefig(path)
        plt.clf()
    ...
    print(results_df)
    results_df.to_excel('skip/mlp_doc2vec.xlsx')