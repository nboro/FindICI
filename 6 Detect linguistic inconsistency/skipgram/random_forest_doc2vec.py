...
class random_forest:
    ...
    def run_rand_forest(self):
        ...
        forest_hyper = grid_search.fit(train_data_vecs, self.y_train)
...
if __name__ == "__main__":
    ...
    for module in top10_list:
        ...
        print('Running RF for the module ', module)
        
        filtered_mutated = get_filtered_mutated(mutated, module)
    
        rand_obj = random_forest(filtered_mutated)

        report_dict, best_params, fpr, tpr, auc, mat = rand_obj.run_rand_forest()
        
        print('The best estimators for the module ', module, ' are: ', best_params)
        
        file_title = 'skip/rf_best_params/rf_doc2vec_best_params_' + module + '.pkl'
        ...
        plt.title('ROC curve')
        plt.legend(loc='best')
        path = 'skip/rf/rf_doc2vec_'+ module + '.jpg'
        plt.savefig(path)
        plt.clf()
    ...
    print(results_df)
    results_df.to_excel('skip/random_forest_doc2vec.xlsx')